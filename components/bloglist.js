...
const PostLink = ({ post, height }) => (
    <article>
        ...
        <div>
            <Link as={`/${post.slug}`} href={`/${post.slug}`}>
                <a href={`/${post.slug}`}>
                    <Image alt={`${getPostTitleWithNames(post)}`} src={getPostImage(post.content.rendered)} width={1140} height={762} placeholder="blur" blurDataURL={getPostImage(post.content.rendered)}/>
                </a>
            </Link>
